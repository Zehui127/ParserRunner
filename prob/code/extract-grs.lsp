#|----------------------------------------------------------------------------|
 | Copyright 2006, 2011, 2012 John Carroll, Ted Briscoe, Rebecca Watson       |
 |                                                                            |
...
(defun add-log-probs (xd yd)
  ; check whether yd so much smaller that won't make any significant contribution
  (if (< (- yd xd) -10.0D0)
    xd
      ...
      (let ((node-set (if (atom queue) queue (car queue))))
        ...
        (dolist (alt (cddar node-set)) 
          (let* ((fs-parent (cadr node-set))
                 ...
                 (alt-prob
                  (reduce #'add-probs fs-parent :key
                     #'(lambda (f-score) (mult-probs f-score alt-e-score))))
              ...
              (dolist (d daughters)
                ...
                (let* ((d-score (caadr d))
                       (f-daughter 
                        (reduce #'add-probs fs-parent :key
                           #'(lambda (f-score)
                               (mult-probs f-score alt-reduce
                                  (div-probs mult-daughters-e-score d-score))))))
                  ;;(format t "~%--------~%daughter: ~%")
                  ;;(pprint d)
                  ...
                  ;; do loop below equivalent but more efficient than following
                  ;; (let ((mem (member d queue :key #'car :test #'eq)))
                  ;;   (if mem
                  ;;     (push f-daughter (cadar mem))
                  ;;     (setq queue (nconc queue (list (list d (list f-daughter)))))))
                  (do ((prev-tail nil tail)
                       (tail queue (cdr tail)))
                     ((null tail)
                       (if prev-tail
                          (setf (cdr prev-tail) (list (list d (list f-daughter))))
                          (setq queue (list (list d (list f-daughter))))))
                     (when (eq (caar tail) d)
                        (push f-daughter (cadar tail))
                        (return)))
