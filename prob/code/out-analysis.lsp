#|----------------------------------------------------------------------------|
 | Copyright 2002, 2006, 2011, 2012, 2017 John Carroll, Ted Briscoe,          |
 |                                        Rebecca Watson, Oeistein Andersen   |
 |                                                                            |
     ...
     (with-output-file-or-stream (out-str out-analyses 
       ...
       (with-output-file-or-stream (log-str logfile
            ...
            (let ((*error-output* log-str)
                  ...
                  (*debug-io* (make-two-way-stream in-str log-str)))
...
(defun init-print-list (&optional arg-o)
  (defun arg-o (flag) 
    (if (find flag arg-o) t nil))
  (setq +print-lemma-lisp+ (arg-o #\l))
  (setq +print-lemma-xml+ (arg-o #\L))
  (setq +print-all+ (arg-o #\p))
  (setq +print-all-xml+ (arg-o #\P))
  (setq +print-list+ nil)
  (push (list 'a (arg-o #\a) (arg-o #\A)
	      nil 'print-tree-output-xml) +print-list+)
  (push (list 's (arg-o #\s) (arg-o #\S)
	      nil
	    'lr1-parse-analysis-upenn-print-xml) +print-list+)
  
  (push (list 'z (arg-o #\z) (arg-o #\Z)
	      nil
	      'lr1-parse-analysis-upenn-print-xml) +print-list+)
  
  (push (list 'u (arg-o #\u) (arg-o #\U)
	      'lr1-parse-analysis-upenn-print
	      'lr1-parse-analysis-upenn-print-xml) +print-list+)
  
  (push (list 'g (arg-o #\g) (arg-o #\G)
	      'lr1-parse-analysis-grs-print
	      'lr1-parse-analysis-grs-print-xml) +print-list+)
  
  (push (list 'gw (arg-o #\w) (arg-o #\W)
	      'lr1-parse-analysis-grs-print-with-weights
	      'lr1-parse-analysis-grs-print-with-weights-xml) +print-list+)
  
  (push (list 'r (arg-o #\r) (arg-o #\R)
              nil nil) +print-list+)
  
  (push (list 't (arg-o #\t) (arg-o #\T)
	      nil
	      'print-tree-output-xml) +print-list+)
  
  ;; important to push last so we can test non-gio switches on!
  (push (list 'gio (arg-o #\i) (arg-o #\I)
...
(defun print-type-fn (type frag-p)
  (let ((print-set (assoc type +print-list+)))
    (or (and frag-p (> (length print-set) 5) (elt print-set 5))
	  (elt print-set 3))))

(defun print-type-xml-fn (type frag-p)
  (let ((print-set (assoc type +print-list+)))
    (or (and frag-p (> (length print-set) 6) (elt print-set 6))
	  (elt print-set 4))))
...
(defun lr1-parse-analysis-trees1 (in-str out-str log-str)
   ...
   (let ((+xml-output+ (print-xml-fn-spec-p))
     ...
     (when (or +xml-output+ +print-lemma-lisp+ +print-lemma-xml+)
       (start-xml out-str))
     (loop
      (let ((sentence1 (read-sentence-from-file in-str out-str log-str)))
       (when (atom sentence1) (force-output out-str) (return))
      ;; sentence1 may contain unpaired quotation marks
      (let ((sentence (sentence-regularise-quotes sentence1)))
	...
	(if (or (print-fn-spec-p) (print-xml-fn-spec-p))
		...
		(let ((frag-p
		  ...
		  (lr1-parse-analysis-output-trees-start (cdr sentence1) trees weights frag-p out-str)
	      ...
	      (setq *current-sentence* 
               (format nil "~:S 0 ; " 
                  (mapcar
                     ...
                     (cdr sentence1))))
	      (xml-start-sentence-print out-str (cdr sentence1) 0 nil)
	      (xml-end-sentence-print out-str)
	      )
	    )
	)))
...
(defun lr1-parse-timeouts nil
   (if (and (integerp +parse-timeout+) (> +parse-timeout+ 0))
      ;; given amount, plus sneak an extra >=2 secs and 100MB upwards to see if
      ...
      (cons
         (cons +parse-timeout+
            ...
            #-(or allegro sbcl) nil)
         (cons
            (max 2 (ceiling +parse-timeout+ 4))
            #+(or allegro sbcl)
            (if (and (integerp +parse-storeout+) (> +parse-storeout+ 0))
               (max 100000000 (ceiling +parse-storeout+ 4))
               nil)
            #-(or allegro sbcl) nil))
