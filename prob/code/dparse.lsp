#|----------------------------------------------------------------------------|
 | Copyright 2002, 2006, 2011, 2012 John Carroll, Ted Briscoe, Rebecca Watson |
 |                                                                            |
...
(defun lr1-compute-word-categories (word word-no)
   (declare (ignore word-no))
   (when (eq word *sentence-end-marker*)
      (return-from lr1-compute-word-categories
         (list (list (position *sentence-end-marker* *terminals* :test #'eq)))))
...
(defun lr1-parse (words)
   (setq *last-lookaheads-list*
      (cons nil
         (let ((word-no 0))
            (mapcar
               #'(lambda (w) (lr1-compute-word-categories w (incf word-no)))
   ...
   (if (member nil (cdr *last-lookaheads-list*) :test #'eq)
       ...
       (let ((nvt-trees nil)
             (root-weights nil))
            (setq *n-unifications* 0)
            (setq *n-failures* 0)
            (setq *last-top-stack* nil)
            (if (and (boundp '*lr1-parse-timeouts*) (consp *lr1-parse-timeouts*))
               (when
                  (eq
                     (let ((+parse-timeout+ (caar *lr1-parse-timeouts*))
                        ...
                        (execute-with-timeout
                           (progn
                              (format *error-output* "~&Halted in parse forest ~A~%"
                                 (if parse-1-p "construction" "unpacking and post-processing"))
                  ...
                  (let ((+parse-timeout+ (cadr *lr1-parse-timeouts*))
                     ...
                     ;; only try to collect partial analyses to give more of a
                     ;; chance of returning something, and also to avoid entering
                     ;; lr1-parse-result-analyses a second time
                     (execute-with-timeout
                        (format *error-output* "~&Halted in partial parse recovery~%")
                        (finish-output *error-output* )
                        (setq *accepted-vertex-analyses* nil)
                        (multiple-value-setq (nvt-trees root-weights) (lr1-parse-2)))))
               (progn
...
(defun lr1-parse-2 ()
   ...
   ;; to best hypothesis
   ; (print (list '> 'lr1-parse-2 (if *accepted-vertex-analyses* t) +lr1-pf-unpack-fn+))
   ; (finish-output)
   (let*
      ((analyses
         (sort
	  (cond
	     (*accepted-vertex-analyses*
	        (funcall +lr1-pf-unpack-fn+)) ; lr1-parse-result-analyses / extract-grs
	     ((not (eq +lr1-pf-unpack-fn+ 'extract-grs)) ; not ewg algorithm
	        (lr1-parse-collect-partial-analyses *last-top-stack*
		   (if *last-lookaheads-list* (butlast *last-lookaheads-list* 1) nil))))
	  #'> :key #'analysis-score))
       (root-weights 
	...
	(mapcar #'analysis-score analyses)))
      ...
      (format t "~&~A~%"
         (mapcar
            ...
            analyses))
      ; (print (list '< 'lr1-parse-2)) (finish-output)
      (values
...
;;; Manage the unpacking process. Use a beam size of +n-best-retained+, and
;;; perform unification consistency verification
...
(defun lr1-parse-result-analyses (&optional (last-verify-fails 0))
   ...
   (let*
      ...
      ;; analyses not guaranteed to be returned in order of scores, and may
      ;; indeed come from more than one alternative parse forest -- so must sort
      ;; them all before pruning to keep best n
      (setq analyses (sort analyses #'> :key #'analysis-score))
      (if (eql +n-best-retained+ 0)
         analyses
         (ldiff analyses (nthcdr +n-best-retained+ analyses)))))
...
(defun lr1-parse-unpack (tree vt rule-cat)
   ...
   (let
     ...
     (loop
      ...
      (let ((packed (cdar tail)))
        (multiple-value-bind (unps unp-scores)
            (lr1-parse-unpack (cddr packed) vt nil)
          (dolist (u unps)
     ...
     (cond
       ...
       (t
        (let ((daughters-alts (list (list tree)))
              (daughters-alts-scores 
                (list (lr1-parse-unpack-score tree (and (consp tail) (car tail))))))
          (dolist (d (cdr tree))
            (multiple-value-setq (daughters-alts daughters-alts-scores)
              (lr1-parse-unpack-daughter d vt nil daughters-alts
                 daughters-alts-scores)))
...
(defun lr1-parse-unpack-daughter (d vt d-cat daughters-alts daughters-alts-scores)
   (let ((unpacked nil) (unpacked-scores nil))
      ...
      (values unpacked unpacked-scores))) ; lr1-parse-unpack-prune
...
(defun lr1-parse-unpack-prune (vt-and-trees scores) 
   ...
   (cond
      ...
      (t
#|
         ;; very slow alternative version which sorts all the scores before pruning
         (let ((pairlis (mapcar #'(lambda (v s) (cons v s)) vt-and-trees scores))
               (vt-and-trees nil)
               (scores nil))
            (setq pairlis (sort pairlis #'> :key #'cdr))
            (dolist (p (ldiff pairlis (nthcdr +n-best-retained+ pairlis)))
               (push (car p) vt-and-trees)
               (push (cdr p) scores))
            (setq vt-and-trees (nreverse vt-and-trees))
            (setq scores (nreverse scores)))
|#
         ;; much faster version without a sort
         (do
            ((threshold
            ...
            ;; if score worse than that of +n-best-retained+th then mark for
            ;; deletion -- but cannot stop after finding +n-best-retained+ since
            ;; input list is not sorted and there may be a few scores tied last
            ;; e.g. n = 3, scores = (4 5 5 5 6)
...
(defun lr1-parse-nth-largest (n scores)
   ...
   (let ((vec 
            (if (and *nth-largest-vector* (eql (length *nth-largest-vector*) n))
               (progn
                  (fill *nth-largest-vector* most-negative-double-float)
                  *nth-largest-vector*)
               (setq *nth-largest-vector*
                  (make-array n :initial-element most-negative-double-float))))
         (end (1- n)))
      (declare (simple-vector vec) (fixnum end))
      (dolist (s scores)
...
;;;                   (format t "- ") (format t "+ ")) ; miss=-, hit=+
               (when (eq cached t)
                  ...
                  (when (and (listp (cdr d)) (> (length (cdr d)) 100)) ; -> hash?
