...
;;; If we're within two memory allocation quanta of the next GC, check if
;;; the amount of heap available before we would reach the maximum heap size
;;; is within a factor of 1.5 of the threshold used to trigger a GC. (If we
;;; were to reach this threshold and not be able to free any memory then the
;;; GC would be invoked repeatedly without any real work being done). If the
;;; check returns true, signal a storeout condition.

#+clozure
(progn
(defparameter +memlimit+ 2000000000) ; 2GB default
(define-condition parse-storeout (simple-condition) ())
(defun check-for-low-memory ()
   (let
      ((remaining
          ;; rough amount of heap left
          ;; don't call ccl::%reservedbytes in 64-bit since returns a negative number
          (- #+:x86-64 +memlimit+ #-:x86-64 (ccl::%reservedbytes)
             (ccl::%usedbytes))))
      (when (< remaining (* (ccl:lisp-heap-gc-threshold) 1.5)) ; factor of 1.5 to be safe
         (signal 'parse-storeout))))
(setq ccl:*pending-gc-notification-hook* 'true) ; outside timed-computation
...
(defmacro execute-with-timeout (timeout-form &body body)
   `(let*
       (.res.
        ...
        (.process.
           (ccl:process-run-function "timed-computation"
              #'(lambda ()
                  (ccl:set-gc-notification-threshold
                     (* (ccl:default-allocation-quantum) 2))
                  (let ((ccl:*pending-gc-notification-hook* 'check-for-low-memory))
                     (handler-case
       ...
       (if (ccl:timed-wait-on-semaphore .sem. +parse-timeout+)
          (if (and .res. (atom .res.))
             (progn
                (format *error-output* "~%Memory exhausted~%")
