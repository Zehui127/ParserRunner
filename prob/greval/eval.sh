...
#NE:
if [ $parc = "560" ]; then
rasp_sentence=cat rasp_morph=cat rasp_ner=$RASP/prob/greval/pn_ner.sh $RASP/scripts/rasp.sh -n -p"-t40 -og" < $RASP/prob/greval/DepBank/parc560/test.ne > $testfile.out
elif [ $parc = "140" ]; then
rasp_sentence=cat rasp_morph=cat rasp_ner=$RASP/prob/greval/pn_ner.sh $RASP/scripts/rasp.sh -n -p"-t40 -og" < $RASP/prob/greval/DepBank/parc140/heldout.ne > $testfile.out
else
rasp_sentence=cat rasp_morph=cat rasp_ner=$RASP/prob/greval/pn_ner.sh $RASP/scripts/rasp.sh -n -p"-t40 -og" < $RASP/prob/greval/DepBank/parc700/all.ne > $testfile.out
fi

else 
# NON NE :
if [ $parc = "560" ]; then
rasp_sentence=cat rasp_morph=cat $RASP/scripts/rasp.sh -p"-t40 -og" < $RASP/prob/greval/DepBank/parc560/test.not-ne > $testfile.out
elif [ $parc = "140" ]; then
rasp_sentence=cat rasp_morph=cat $RASP/scripts/rasp.sh -p"-t40 -og" < $RASP/prob/greval/DepBank/parc140/heldout.not-ne > $testfile.out
else
rasp_sentence=cat rasp_morph=cat $RASP/scripts/rasp.sh -p"-t40 -og" < $RASP/prob/greval/DepBank/parc700/all.not-ne > $testfile.out
fi
...
grep -v "()" $RASP/prob/greval/DepBank/parc700/sents-ne-all.gr > $RASP/prob/greval/DepBank/parc700/sents-ne-all.gr2
mv $RASP/prob/greval/DepBank/parc700/sents-ne-all.gr2 $RASP/prob/greval/DepBank/parc700/sents-ne-all.gr

else

awk -v RASP=$RASP  'BEGIN{hld=" 1 6 11 16 21 26 31 36 41 46 51 56 61 66 71 76 81 86 91 96 101 106 111 116 121 126 131 136 141 146 151 156 161 166 171 176 181 186 191 196 201 206 211 216 221 226 231 236 241 246 251 256 261 266 271 276 281 286 291 296 301 306 311 316 321 326 331 336 341 346 351 356 361 366 371 376 381 386 391 396 401 406 411 416 421 426 431 436 441 446 451 456 461 466 471 476 481 486 491 496 501 506 511 516 521 526 531 536 541 546 551 556 561 566 571 576 581 586 591 596 601 606 611 616 621 626 631 636 641 646 651 656 661 666 671 676 681 686 691 696 "; \
            sent=0; senth=0; sentt=0; rasp=0} \
  function output_gr() {\
    nmatches=0; \
    for (v in proper) {if (index($0,v)) {nmatches++}}; \
