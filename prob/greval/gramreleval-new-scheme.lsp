   ...
   (let ((*gr-state-orig (make-gr-state :nsents 0)) 
	 ...
	 (cond
	   ...
	   (when *output-orig
	     ...
	     (format t "~%ORIGINAL PERFORMANCE:~%")
	     (greval-relation-summary t *gr-state-orig))
	   ...
	   (format t "~%NEW SCHEME PERFORMANCE:~%")
	     ...
	     (format t "~%UNLABELLED DEPENDENCY PERFORMANCE:~%")
	     (greval-summary t *gr-state-unlabelled))
      ...
      (let ((std (greval-plist-+ (gr-state-std-total s)))
         ...
         (let
            ...
            (format out-str "~4D           ~7,2F         ~7,2F          ~
                             ~7,2F        ~9:D~%"
               (gr-state-nsents s) p r (greval-f-score p r) (round tst)))))
	 ...
	 (dolist (desc-rel
	   ...
	   ;; print out sum of all sub-cat too
           (when (not (eq *type 'hier))
             (format 
	      out-str 
	      "~20A ~10,2F ~10,2F ~10,2F ~10:D ~10:D ~10:D~%"
	      (format nil "~VA~A"
		      (1+ (cdr (assoc rel +relation-depth-table+))) " " rel)
	      p-all r-all (greval-f-score p-all r-all) (round tst) (round std) (round agree)))
	   ;; print out exact match for particular relation too! (if any)
		  ...
		  (format nil "~VA~A"
			  ...
			  " " rel)
		  (format nil "~VA~A"
			  (+ 2 (cdr (assoc rel +relation-depth-table+))) 
			  " " "--")) 
	      p r (greval-f-score p r) (round tst-i) (round std-i) (round agree-i)))
	   ))))
     (greval-summary out-str gr-state))
