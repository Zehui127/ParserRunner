...
awk 'BEGIN{printf("%%LB (\n%%RB )\n"); sent=0} \
  ...
  /^[(]/ { \
    gsub(/[|\\]/,""); 
    while ((where=match($0, / [^< ]*<[\/]w>/))!=0) \
      {$0 = substr($0, 1, where-1) "_" substr($0, where+1)} \
    gsub(/<[\/]?w>/,""); \
...
(load (format nil "$RASP/prob/greval/gramreleval-new-scheme.lsp"))
  ...
  (flet ((lr1-parse-unpack-word (fm &optional (numberwords t))
	   (let* ((form (string fm)))
		 ...
		 ((epos (or (and numberwords
                                 (position #\: form :from-end t :end (1- (length form))))
