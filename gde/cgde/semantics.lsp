   ...
   (when
      (and n-and-category
         (match-category 0 (cdar form) 0
            (or (semantic-form-getf *cached-converted-categories* (cadr n-and-category))
               (setf
...
(defun semantic-form-getf (lst key)
   (tagbody
      (go end)
      lp
      (when (eq key (car lst))
         (return-from semantic-form-getf (cadr (the list lst))))
      (setq lst (cddr (the list lst)))
      end
      (if lst (go lp) (return-from semantic-form-getf nil))))
