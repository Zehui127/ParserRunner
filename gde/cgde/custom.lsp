...
;;; overheads (or zero if not known).
...
(defun conscount nil
   ...
   #+CLOZURE (ccl::%heap-bytes-allocated)
   #-(or XEROX PROCYON CMU SBCL CLOZURE) 0)


(defun total-gc-time-msecs nil
   #+XEROX (il:clock 3)
   #+PROCYON
   (floor (* (system:%gctime t) 1000) internal-time-units-per-second)
   #+MCL (gctime)
   #+CLOZURE (floor (* (gctime) 1000) internal-time-units-per-second)
   #-(or XEROX PROCYON MCL CLOZURE) 0)
