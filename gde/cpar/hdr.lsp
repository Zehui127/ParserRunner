...
(defmacro g-varp (x)
   ...
   `(locally
      ...
      (declare (optimize (speed 3) (safety 0) (space 0))
         (inline symbolp))
      #+clozure (locally (declare (inline ccl:structurep)) (ccl:structurep ,x))
      #-clozure (not (symbolp ,x))))
...
(defmacro g-categoryp (x)
   ...
   #+(or clozure openmcl mcl)
      `(arrayp ,x)
   #-(or (and allegro (not (or cltl2 x3j13))) kcl clozure openmcl mcl)
      `(simple-vector-p ,x)) ; 
   )
