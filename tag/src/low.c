...
static int get_corpus_word(FILE *fp, uchar *text, int max)
{
	...
	/* Start of XML w element?
           Add 2 to nesting level; decremented by the matching */
	/* close w in two steps */
        if (ch == '<' && *line_ptr == 'w' &&
	                 (Local_isspace(*(line_ptr+1)) || *(line_ptr+1) == '>'))
	{
	    xnest+=2;
	}
	
	if (ch == '<' && *line_ptr == '/' && *(line_ptr+1) == 'w')
	{
	    xnest--;
	}

	if (ch == '>' && (xnest & 1)) /* ending a close w tag */
	{
	    xnest--;
	}

	/* End of sentence anchor? If yes we accumulate text until next start anchor */
        if (i == 1 && ch == '^' && *line_ptr == '^')
	{
	    while (i < max)
	    {
		if (*line_ptr == 0 && !get_corpus_line(fp)) return 0;

		ch = (*line_ptr == ' ' && *(line_ptr+1) == 0) ? '\n' : *line_ptr;
		if (ch == '^' && i > 2) return 0;
		text[i++] = ch;
		text[i] = 0;
		line_ptr += 1;
...
BOOL number_word(uchar *text, uchar *scrtext)
{
    ...
    for ( ;text[i] != '\0'; )
    {
        if (text[i] == '<') inside_tag = TRUE;
	if (! inside_tag)
	  scrtext[j++] = text[i];
	if (text[i] == '>') inside_tag = FALSE;
	i++;
    }
    scrtext[j] = '\0';
...
BOOL translate_xml_or_number(uchar *intext, uchar *outext)
{
    ...
    for ( ;intext[i] != '\0'; )
    {
	if (intext[i] == '<') translated = inside_tag = TRUE;
	if (! inside_tag)
	{
