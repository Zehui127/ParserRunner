/******************************************************************************
 * Copyright 1995, 2002, 2006, 2011, 2012, 2015                               *
 * John Carroll, Guido Minnen, Larry Piano, David Elworthy, Oeistein Andersen *
 *                                                                            *
...
void create_trans(Trans *t, int size)
{
    ...
    if (t->gamma == NULL)
	/* Ensure array size of at least 400 (constant used in mainl.c). */
	Allocate(t->gamma, (size<400 ? 400 : size) * sizeof(Score), "gamma");
